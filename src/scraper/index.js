...
const getProblem = (url) => {
  ...
  return axios.get(url)
    .then(res => {
      ...
      // contest name of the problem
      data.contestName = $('#sidebar > .roundbox.sidebox:first-child th').text();
      ...
      data.inSpecsHtml = $('.problem-statement > .input-specification').html();

      // output constraints and specs html
      data.outSpecsHtml = $('.problem-statement > .output-specification').html();
      ...
      $('.problem-statement > .sample-tests > .sample-test').children().each((i, ele) => {
