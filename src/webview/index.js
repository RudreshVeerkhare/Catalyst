...
const createWebview = async (problemData, context) => {
    ...
    if (currentPanel === undefined) {
        ...
        // populating webview
        currentPanel.webview.html = utils.getWebviewContent(
            context,
            problemData
        );
    }
...
const runCases = async (problemData, id = null) => {
    ...
    try {
        const compileExitCode = await codeRunner.compiler.compileSourceCode(
            problemData
        );
    } catch (err) {
    ...
    for (let testcase of cases) {
        ...
        const result = await codeRunner.runner.runTestCase(
            testcase,
            language,
            timeout,
            filePath
